...
class LitModel(pl.LightningModule):
    ...
    def encode(self, x, t=None, return_2d_feature=False):
        # TODO:
        assert self.conf.model_type.has_autoenc()
        cond = self.ema_model.encoder.forward(x, t, return_2d_feature)
        return cond
    ...
    def setup(self, stage=None) -> None:
        ...
        # self.train_data = self.conf.make_dataset(path="./datasets/mvtec_train.lmdb")
        self.train_data = self.conf.make_dataset(path="D:\\xhh\\diffae-multi-gpus\\datasets\\chl.lmdb")
        print('train data:', len(self.train_data))
        # self.val_data = self.conf.make_dataset(path="./datasets/mvtec_val.lmdb")
        # self.val_data = self.conf.make_dataset(path="D:\\xhh\\diffae-multi-gpus\\datasets\\mvtec_val.lmdb")
        ...
        def do(model,
            ...
            with torch.no_grad():
                ...
                if self.global_rank == 0:
                    ...
                    # make_grid(): 按照行优先排列，默认每行(nrow)8张图像
                    grid = (make_grid(gen) + 1) / 2  # [-1, 1] -> [0, 1]
...
def train(conf: TrainConfig, gpus, nodes=1, mode: str = 'train'):
    ...
    # checkpoint_path = f'{conf.logdir}/epoch2192-step1849926.ckpt'
