...
# 数据类
# https://www.51cto.com/article/719951.html
@dataclass
class TrainConfig(BaseConfig):
    ...
    postfix: str = ''
    sample_size: int = 64
    sample_every_samples: int = 100_000  # 隔多久采样一次
    save_every_samples: int = 100_000
    style_ch: int = 512
    T_eval: int = 1_000  # 验证时的循环采样步数
    T_sampler: str = 'uniform'
    T: int = 1_000
    total_samples: int = 100_000_000
    warmup: int = 0
    ...
    # 如果要初始化一个依赖于另一个属性值的属性，可以使用__post_init__()。
    # 顾名思义，Python 在 __init__()之后调用 __post_init__()。
